    ...
    with open(vocab.settings_file, "r", encoding='utf-8') as file:
        ...
        for word in re.findall(r' *(.+?) *= *(.+) *', content):
            if word[0].strip() == 'result_file':
                try:
                    vld.validate_filename_format(vocab.result_file.parent, word[1].strip())
                    vld.validate_file_name_unique(word[1].strip(), vocab.directories_file.name)
                ...
                try:
                    vld.validate_filename_format(vocab.directories_file.parent, word[1].strip())
                    vld.validate_file_name_unique(word[1].strip(), vocab.result_file.name)
                ...
                try:
                    ...
                    logger.warning(f"Invalid processing option '{word[0].strip()}': {e}")
            elif word[0].strip() == 'use_dir_with_leading_exclamation_mark':
                try:
                    vld.validate_bool_value(cns.BOOLEAN_STRINGS, word[1].strip())
                    vocab.use_dir_with_leading_exclamation_mark = (word[1].strip().lower() in cns.TRUTH_STRINGS)
                    try:
                        # If the PyInstaller executable sets the base directory to '.'
                        if vocab.base_directory.name == '':
                            cur_list = re.split(r'[\\/]', str(vocab.base_directory.resolve()))
                            dirpath_name = cur_list[len(cur_list) - 1]
                        else:
                            dirpath_name = vocab.base_directory.name
                        message = "The 'use_dir_with_leading_exclamation_mark' option is set to false, but the base directory has the leading exclamation mark"
                        vld.validate_directory_with_leading_exclamation_mark(dirpath_name, vocab.use_dir_with_leading_exclamation_mark, message)
                    except exc.VocabError as e:
                        logger.warning(e)
                except exc.VocabError as e:
                    logger.warning(f"Invalid processing option '{word[0].strip()}': {e}")