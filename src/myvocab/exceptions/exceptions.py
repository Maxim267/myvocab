...
class FetchIAMtokenError(VocabError):
    """ Raised when failing to fetch the IAM token. """

    def __init__(self, extra_message, message="Failed to fetch IAM token"):
        self.extra_message = extra_message
        self.message = message
        super().__init__(self.message)

    def __str__(self) -> str:
        return f"{self.message} {self.extra_message}"

class FetchSupportedLanguagesError(VocabError):
    """ Raised when failing to fetch the supported languages. """

    def __init__(self, extra_message, message="Failed to fetch the supported languages"):
        self.extra_message = extra_message
        self.message = message
        super().__init__(self.message)
...
class TargetLanguageCodeIsNotFoundError(VocabError):
    """ Raised when a target language is not found. """

    def __init__(self, target_language, message="The target language code is not found:"):
        self.target_language = target_language
        self.message = message
        super().__init__(self.message)

    def __str__(self) -> str:
        return f"{self.message} `{self.target_language}`"